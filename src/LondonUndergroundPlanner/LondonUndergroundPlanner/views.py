...
from .coursework_code.excel_manipulation import excel_manipulation, dlr_excel_manipulation
from .coursework_code.algorithms import dijkstras_algorithm, find_route_lines, change_line_info
...
excel_file = openpyxl.load_workbook(
    'LondonUndergroundPlanner\\coursework_code\\TrainDataWithDLR.xlsx'
)
## Excel import below is for original dataset. 
# excel_file = openpyxl.load_workbook(
#     'LondonUndergroundPlanner\\coursework_code\\TrainData.xlsx'
# )
...
class HomeView(TemplateView):
    ...
    def get(self, request):
        ...
        """
        # Form instance created from forms.py (so user can choose input in comboboxes)
        form = JourneyForm()
    ...
    def post(self,request):
        ...
        if form.is_valid():
            ...
            ## line 94 commented out our program is currently using dataset with DLR information
            # station_dll = excel_manipulation(sheet, off_peak)     # var for non-dlr doubly_linked_list
            station_dll = dlr_excel_manipulation(sheet, off_peak)   # var for dlr doubly_linked_list
